...
\midrule
\textbf{UMSG Network (Ours)} & \textbf{Unified Parallel Manifolds} & \textbf{Anchor-Free Probability Field} & $\mathbf{\mathcal{O}(N \cdot K + (K + M)^2 \cdot D)}$ & \textbf{Native Hidden Core Fusion} \\
\bottomrule
\end{tabular*}
\end{table*}
"""
    ...
    print(f"[SUCCESS] LaTeX Table 1 generated and saved to '{filepath}'")
